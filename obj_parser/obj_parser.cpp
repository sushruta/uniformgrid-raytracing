...
int obj_parse_mtl_file(char *filename, list *material_list)
{
	...
	while( fgets(current_line, OBJ_LINE_SIZE, mtl_file_stream) )
	{
		...
		else if(strequal(current_token, "Ka") && material_open)
		{
			...
			//printf("straight from file : (%f, %f, %f)\n", current_mtl->amb[0], current_mtl->amb[1], current_mtl->amb[2]);
			//my_mat.amb[0] = current_mtl->amb[0];
			//my_mat.amb[1] = current_mtl->amb[1];
			//my_mat.amb[2] = current_mtl->amb[2];
		}
