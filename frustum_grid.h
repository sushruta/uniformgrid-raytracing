#ifndef FRUSTUM_GRID_H
#define FRUSTUM_GRID_H
...
{
	public:
		FrustumGrid(int TRI_COUNT);
		~FrustumGrid();

		void buildGrid(int *d_facelist, float *d_vertlist);
		void buildSphericalGrid(int *d_facelist, float *d_vertlist, float xM, float yM);
		void printSpan();
...
FrustumGrid::FrustumGrid(int TRICOUNT)
{
	...
	h_span = (unsigned int *) malloc(sizeof(unsigned int) * NUM_BLOCKS_X * NUM_BLOCKS_Y * NUM_SLABS);
...
void FrustumGrid::init_cudpp_plans()
{
	config.op = CUDPP_ADD;
...
int FrustumGrid::getMinMax_host(float *zmin, float *zmax)
{
...
void FrustumGrid::buildGrid(int *d_facelist, float *d_vertlist)
{
	...
	cudaThreadSynchronize();
	create_histogram<<< histGridSize, histThreadSize >>>(d_compacted_list, (int) *h_num_actual_cells, total_triangles, d_triangle_key_list, d_span);
	cudaThreadSynchronize();
...
void FrustumGrid::buildSphericalGrid(int *d_facelist, float *d_vertlist, float xM, float yM)
{
	printf("blocks : %d\tthreads : %d\n", NUMBLOCKSDS, NUMTHREADSDS);
	dim3 dsGrid(NUMBLOCKSDS, 1, 1);
	dim3 dsThreads(NUMTHREADSDS, 1, 1);
	...
	cudaThreadSynchronize();
	//DSKernel<<< dsGrid, dsThreads >>>(d_sizeList, d_projCoordZ, d_facelist, d_vertlist, dd_modelParams);
	DS_spherical_Kernel<<< dsGrid, dsThreads >>>(d_sizeList, d_projCoordZ, d_facelist, d_vertlist, dd_modelParams, xM, yM);
	cudaThreadSynchronize();
	...
	// do the reduction here
	printf("NUMELEMENTS : %d\n", TRI_COUNT);
	float zMin = 9999.9f, zMax = -9999.9f;
	int elems = 0;
	...
	/*unsigned int *h_s = (unsigned int *) malloc(sizeof(unsigned int) * TRI_COUNT);
	cutilSafeCall(cudaMemcpy(h_s, d_sizeList, sizeof(unsigned int) * TRI_COUNT, cudaMemcpyDeviceToHost));
	for (int i=0; i<TRI_COUNT; i++)
		printf("value at %d : %d\n", i, h_s[i]);
	free(h_s);*/
	...
	cudaThreadSynchronize();
	//DSFillkernel<<< dsGrid, dsThreads, sharedmem >>>(d_triangle_key_list, d_triangle_value_list, d_scannedSizeList, d_zSlabs, d_facelist, d_vertlist, dd_modelParams);
	DS_spherical_Fillkernel<<< dsGrid, dsThreads, sharedmem >>>(d_triangle_key_list, d_triangle_value_list, d_scannedSizeList, d_zSlabs, d_facelist, d_vertlist, dd_modelParams, xM, yM);
	cudaThreadSynchronize();
...
void FrustumGrid::printSpan()
{
	cutilSafeCall(cudaMemcpy(h_span, d_offset, sizeof(unsigned int) * NUM_BLOCKS_X * NUM_BLOCKS_Y * NUM_SLABS, cudaMemcpyDeviceToHost));
	for (int i=0; i<NUM_BLOCKS_X * NUM_BLOCKS_Y * NUM_SLABS; i++)
	{
		printf("%d\n", h_span[i]);
	}
}
